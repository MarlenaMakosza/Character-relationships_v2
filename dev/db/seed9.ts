...
try {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition,@typescript-eslint/no-magic-numbers -- the code cannot work when GROUP_SIZE other than 2.
  if (GROUP_SIZE != 2) {
    throw new Error(
      // eslint-disable-next-line @typescript-eslint/restrict-template-expressions -- this code is meant as an assertion for unlikely case when someone redefines GROUP_SIZE to anything other than 2.
