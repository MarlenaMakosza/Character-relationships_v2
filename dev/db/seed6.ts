...
try {
  ...
  type SeededCharacter = typeof characters.$inferSelect; // drizzle type not from domain
  type CharacterPair = [SeededCharacter, SeededCharacter];
  const TWO = 2;
  const PAIR = TWO satisfies CharacterPair['length'];
  ...
  await db.insert(relations).values(
    Array.from({ length: RELATION_COUNT }, () => {
      const charInPair = faker.helpers.arrayElements(seededCharacters, PAIR) as CharacterPair; // <- here: cast to local type, not Character from domain
